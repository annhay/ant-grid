...
import (
	inits "ant-grid/internal/common/init"
	"ant-grid/internal/proto"
	"ant-grid/internal/server-order/scheduler"
	"ant-grid/internal/server-order/server"
	"flag"
	"log"
	"net"
	"os"
...
func main() {
	...
	// 启动订单定时任务
	scheduler.StartOrderScheduler()

	// 注册到 Consul
	consul := inits.NewConsul("14.103.173.254:8500")
	kv := inits.ConsulKV{
		Name:    "order-server",
